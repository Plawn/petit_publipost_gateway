...
class Templator:
    ...
    def render(self, template_name: str, data: Dict[str, str], tables_data:Dict[str, Dict[str, str]],output_name: str) -> str:
        res: Dict[str, str] = {}
        for _type, val in data.items():
            for key, value in val.items():
                res[self.class_separator.join((_type, key))] = value
        doc = self.templates[template_name].apply_template(res, tables_data)
...
class TemplateDB:
    ...
    def render_template(self, _type: str, name: str, data: Dict[str, str], tables_data:Dict[str, Dict[str, str]], output: str):
        return self.templators[_type].render(name, data, tables_data,output)
